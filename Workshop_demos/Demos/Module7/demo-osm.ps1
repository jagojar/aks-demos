# Namespace should be kube-system if AKS plugin installed.  osm-system otherwide (assuming the default namespace 
# was used when installing OSM)
...
function CleanUp ([string] $done) {
    ...
    kubectl patch meshconfig osm-mesh-config -n $namespace --patch-file .\patch-enablePermissiveTraffic.yaml --type=merge
...
kubectl patch meshconfig osm-mesh-config -n $namespace --patch-file .\patch-disablePermissiveTraffic.yaml --type=merge
